...
function parseWeddingDate(raw) {
  ...
  return Number.isNaN(maybe.getTime()) ? new Date('2026-05-10T17:30:00+07:00') : maybe
}
...
export default function HomePage() {
    ...
    observerRef.current = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          setAnimateElements(prev => ({ ...prev, [entry.target.id]: true }))
        }
      })
  ...
  const wishes = data.wishes?.length ? data.wishes : [
    { id: 1, fullName: 'Cô dâu chú rể', message: 'Chúc tình yêu bền lâu, dịu dàng và hạnh phúc mãi mãi. ❤️', createdAt: '2024-01-15' },
    { id: 2, fullName: 'Gia đình', message: 'Chúc hai con luôn yêu thương, trân trọng và nắm tay nhau thật lâu.', createdAt: '2024-01-20' },
    { id: 3, fullName: 'Bạn bè', message: 'Một hành trình mới thật đẹp đang bắt đầu. Chúc hai bạn thật viên mãn.', createdAt: '2024-01-25' }
  ]
  ...
  const openMap = useCallback((url) => {
    window.open(url || 'https://maps.app.goo.gl/4E64mXSbZVzMZ6DQ8', '_blank')
  }, [])
    ...
    <div className={styles.app}>
      <nav className={`${styles.navbar} ${scrolled ? styles.scrolled : ''}`}>
        <div className={styles.navContainer}>
          ...
          <ul className={styles.navMenu}>
            <li className={activeNav === 'loveStory' ? styles.active : ''} onClick={() => scrollToSection('loveStory')}>
              Chuyện tình
            </li>
            <li className={activeNav === 'venues' ? styles.active : ''} onClick={() => scrollToSection('venues')}>
              Địa điểm
            </li>
            <li className={activeNav === 'rsvp' ? styles.active : ''} onClick={() => scrollToSection('rsvp')}>
              Xác nhận
            </li>
          </ul>
        </div>
      ...
      <main>
          ...
          <div className={styles.container}>
            <div className={styles.storyGrid}>
              <div className={`${styles.glassCard} ${styles.storyCopy} ${animateElements.loveStory ? styles.fadeInUp : ''}`} data-animate id="loveStory">
                ...
                {(site.story || 'Duy Trung và Thu Trang – hai mảnh ghép tưởng chừng xa lạ lại tìm thấy nhau giữa vô vàn điều đẹp đẽ của cuộc sống. Từ những lần gặp gỡ bình dị, những cuộc trò chuyện dài và sự đồng hành chân thành, cả hai dần nhận ra mình chính là bến bờ bình yên của nhau.\n\nHành trình yêu thương được viết bằng những chuyến đi, tiếng cười và sự thấu hiểu. Và giờ đây, tình yêu ấy sẽ được đơm hoa trong ngày trọng đại, nơi gia đình và bạn bè cùng chứng kiến.').split(/\n+/).map((paragraph, index) => (
        ...
        <section className={styles.section}>
          <div className={styles.container}>
            <div className={styles.countdownWrap}>
              <h2 className={styles.sectionTitle} data-animate id="countdown">Ngày trọng đại</h2>
              <p className={styles.countdownDesc}>Chúng tôi rất mong được gặp bạn trong ngày hạnh phúc nhất của mình.</p>
              <div className={styles.timer}>
                {[
                  ...
                  <div className={`${styles.timerCard} ${animateElements.countdown ? styles.bounceIn : ''}`} data-animate id="countdown" key={label} style={{ animationDelay: `${idx * 0.1}s` }}>
        ...
        <section className={styles.section} id="venues">
          <div className={styles.container}>
            <h2 className={styles.sectionTitle} data-animate id="venues">Lễ cưới &amp; Tiệc mừng</h2>
            <p className={styles.sectionSubtitle}>Mọi yêu thương sẽ hội tụ trong một ngày thật đẹp, tại những nơi lưu giữ khoảnh khắc thiêng liêng nhất của chúng tôi.</p>

            <div className={styles.venuesGrid}>
              {venueItems.map((venue, idx) => (
                <div className={`${styles.venueCard} ${animateElements.venues ? styles.slideInUp : ''}`} data-animate id="venues" key={idx} style={{ animationDelay: `${idx * 0.15}s` }}>
                  <div className={styles.venueIcon}><i className={venue.icon} /></div>
                  <h3>{venue.title}</h3>
                  <p>
              ...
              <div className={`${styles.venueCard} ${animateElements.venues ? styles.slideInUp : ''}`} data-animate id="venues" style={{ animationDelay: '0.3s' }}>
                <div className={styles.venueIcon}><i className="fa-solid fa-qrcode" /></div>
                <h3>Mã QR check-in</h3>
                <p>{site.qrDescription || 'Quét mã tại cửa để nhận ảnh cưới kỷ niệm và món quà lưu niệm nhỏ từ cô dâu chú rể.'}</p>
                <button
                  ...
                  onClick={() => {
                    if (site.qrImageUrl) {
                      ...
                      alert('Mã QR sẽ được cập nhật sớm.')
                    }
                  }}
                >
            ...
            <div className={styles.container}>
              ...
              <div className={styles.galleryGrid}>
                {gallery.map((item, index) => (
                  <div className={`${styles.galleryItem} ${animateElements.gallery ? styles.scaleIn : ''}`} data-animate id="gallery" key={item.id || index} style={{ animationDelay: `${index * 0.05}s` }}>
        ...
        {site.showMusic !== false && (
          <section className={styles.section}>
            <div className={styles.container}>
              <div className={`${styles.playlistCard} ${animateElements.music ? styles.fadeInUp : ''}`} data-animate id="music">
                <div className={styles.playlistIcon}><i className="fa-solid fa-headphones" /></div>
                ...
                <div className={styles.playlistTags}>
                  {(site.songs?.length ? site.songs : defaultSongs).map((song, idx) => (
                    <div className={styles.playlistTag} key={idx}>
                      <i className="fa-solid fa-music" /> {typeof song === 'string' ? song : `${song.name} - ${song.artist}`}
                    </div>
                  ))}
                </div>
                <button
                  ...
                  onClick={() => {
                    if (site.backgroundMusicUrl) window.open(site.backgroundMusicUrl, '_blank')
                    else alert('Chúc mừng bạn đã đến với playlist tình yêu của chúng tôi! 💖')
                  }}
                >
                  <i className="fa-brands fa-spotify" /> Nghe trên Spotify
                </button>
              </div>
            </div>
          </section>
        ...
        <section className={`${styles.section} ${styles.sectionSoft}`}>
          <div className={styles.container}>
            <div className={styles.thanksGrid}>
              <div className={`${styles.thanksMedia} ${animateElements.thanks ? styles.slideInLeft : ''}`} data-animate id="thanks">
                <img src={site.thanksImageUrl || 'https://picsum.photos/id/64/900/1100'} alt="Lời tri ân" />
              </div>
              <div className={`${styles.glassCard} ${styles.thanksCopy} ${animateElements.thanks ? styles.slideInRight : ''}`} data-animate id="thanks">
                <h2 className={styles.sectionTitleLeft}>Lời tri ân</h2>
                <div className={styles.sectionDividerLeft}><i className="fa-solid fa-heart" /></div>
                <p>Gửi đến ông bà, cha mẹ, người thân và bạn bè yêu quý — cảm ơn vì tất cả sự yêu thương, chở che và đồng hành để chúng con trưởng thành như hôm nay.</p>
                <p>Cảm ơn những cái nắm tay, những lời chúc, những sự hiện diện âm thầm nhưng ấm áp trong suốt hành trình yêu thương của chúng con.</p>
                <p>Sự hiện diện của mọi người trong ngày trọng đại này chính là món quà quý giá nhất, là kỷ niệm mà chúng con sẽ luôn gìn giữ.</p>
                <p className={styles.quote}>“Tình yêu không chỉ là nhìn về phía nhau, mà còn cùng nhau nhìn về một hướng.”</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className={styles.section} id="rsvp">
          <div className={styles.container}>
            <h2 className={styles.sectionTitle} data-animate id="rsvpForm">Xác nhận tham dự</h2>
            <p className={styles.sectionSubtitle}>Hãy để lại lời xác nhận và những lời chúc ngọt ngào dành cho cô dâu chú rể.</p>

            <div className={`${styles.rsvpShell} ${animateElements.rsvpForm ? styles.fadeInUp : ''}`} data-animate id="rsvpForm">
    ...
    <div className={styles.bankGrid}>
      {bankAccounts.map((bank) => (
        ...
        {site.showWishes !== false && (
          <section className={`${styles.section} ${styles.sectionSoft}`}>
            <div className={styles.container}>
              <h2 className={styles.sectionTitle} style={{ marginBottom: 0 }}>Lời chúc ngọt ngào</h2>
              <p className={styles.sectionSubtitle} style={{ marginBottom: 20 }}>Những lời chúc nhỏ sẽ làm ngày vui của chúng tôi thêm trọn vẹn.</p>

              <div className={styles.guestMessages}>
                {wishes.slice(0, 9).map((item, index) => (
                  <div className={`${styles.wishCard} ${animateElements.wishes ? styles.fadeInUp : ''}`} data-animate id="wishes" key={item.id || index} style={{ animationDelay: `${index * 0.05}s` }}>
        ...
        <div className={styles.modalOverlay} onClick={() => setWishModalOpen(false)}>
          <div className={styles.modalContainer} onClick={(e) => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <h3>Gửi lời chúc</h3>
              <button className={styles.modalClose} onClick={() => setWishModalOpen(false)}>×</button>
            </div>
            ...
            <form onSubmit={submitWish}>
              ...
              <div className={styles.modalActions}>
                <button type="submit" className={styles.submitBtn}>Gửi ngay</button>
                <button type="button" className={styles.btnOutline} onClick={() => setWishModalOpen(false)}>Đóng</button>
              </div>
            </form>
          </div>
        </div>
